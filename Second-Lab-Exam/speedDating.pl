...
kConsecutiveSlots(MaxSlots,K,L):- SMaxStart is MaxSlots-K+1, between(1,SMaxStart,SStart),
				  findall(X, (between(1,K,Pos), X is SStart+Pos-1), L).
...
noIncompatibleDatesAtSameSlot(MaxSlots):- date(P1-P2), date(P3-P4), shareParticipant(P1-P2, P3-P4), slot(S), writeOneClause([-pps(P1-P2, S), -pps(P3-P4, S)]), fail.
noIncompatibleDatesAtSameSlot(_).

relatePSwithPPS(MaxSlots):- date(P1-P2), slot(S), writeOneClause([-pps(P1-P2, S), ps(P1,S)]), writeOneClause([-pps(P1-P2, S), ps(P2, S)]), fail.
relatePSwithPPS(_).
% 1 2 3 4 5 6 7 8
% MAxslots is 2
%
restEveryKConsecutiveDates(MaxSlots):- participant(P), maxConsecutiveDates(K), K1 is K+1, slot(S), S > K, L is S - K, findall(-ps(P, Q), between(L, S, Q), Lits), atLeast(1, Lits), fail.
restEveryKConsecutiveDates(_).

maxDatesAtTheSameSlot(MaxSlots):- numTables(T), slot(S), findall(pps(P1-P2, S), date(P1-P2),Lits), atMost(T, Lits), fail.
maxDatesAtTheSameSlot(_).

noDateOverMaxSlots(MaxSlots) :- slot(S), S > MaxSlots, findall(pps(P1-P2, S),date(P1-P2), Lits), exactly(0, Lits), findall(ps(P, S), participant(P), Lits2), exactly(0, Lits2), fail.
noDateOverMaxSlots(_).

betweenVipsBefore(MaxSlots):- vips(L), date(P1-P2), not(member(P1, L)), not(member(P2, L)), slot(S), member(V1, L), member(V2, L), date(V1-V2), slot(T), T > S, writeOneClause([-pps(P1-P2, S), -pps(V1-V2, T)]), fail.
betweenVipsBefore(_).
...
costOfThisSolution(M,Cost):- between(0, 15, I), Cost is 15 - I, findall(S, member(ps(_, S), M), Lits), sort(Lits, Lits2), getMaxElem(Lits2, Cost), !.


getMaxElem([], 0).
getMaxElem([A|B], A) :- getMaxElem(B, X), A >= X.
getMaxElem([A|B], X) :- getMaxElem(B, X), X >= A.
