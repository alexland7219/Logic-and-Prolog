...
satVariable(  ccd(C1, C2, D) ):- day(D), trip(C1-C2).          % C1-C2 is done on day D

%%%%%%%  2. Clause generation for the SAT solver: =============================================

writeClauses:-  
    exactlyOneTripPerBusAndDay,
    eachTrajectAtMostOncePerDay,
    busDepartsNextDay,
    minDaysPerTrip,
    linkCCD,
    linkCCD2,
...
eachTrajectAtMostOncePerDay :- day(D), trip(C1-C2), findall(bdcc(B, D, C1, C2), bus(B), Lits), atMost(1, Lits), fail.
eachTrajectAtMostOncePerDay.

busDepartsNextDay :- bus(B), numDays(X), day(D1), D1 < X, consecutiveDays(D1, D2), trip(I-F), findall(bdcc(B, D2, F, FF), trip(F-FF), Lits), atLeast(1, [-bdcc(B, D1, I, F) | Lits]), fail.
busDepartsNextDay.

minDaysPerTrip :- trip(C1-C2), frequency(C1-C2, X), findall(ccd(C1, C2, D), day(D), Lits), atLeast(X, Lits), fail.
minDaysPerTrip.

linkCCD :- bus(B), day(D), trip(C1-C2), atLeast(1, [-bdcc(B, D, C1, C2), ccd(C1, C2, D)]), fail.
linkCCD.

linkCCD2 :- trip(C1-C2), day(D), findall(bdcc(B, D, C1, C2), bus(B), Lits), atLeast(1, [-ccd(C1, C2, D) | Lits]), fail.
linkCCD2.

maxKms :- bus(B), numDays(X), day(D1), D1 < X, tooLongDist(C1-C2-C3), consecutiveDays(D1, D2), atLeast(1, [-bdcc(B, D1, C1, C2), -bdcc(B, D2, C2, C3)]), fail.
maxKms.
