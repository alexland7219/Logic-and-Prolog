...
incompatibleWorkers(W1,W2):- W1 \= W2, maxScore(K), workerScore(W1, M1), workerScore(W2, M2), K < M1 + M2.
...
eachWorkerOneTeam :- worker(W), findall( wt(W,T), team(T) , Lits ), exactly(1, Lits), fail.
eachWorkerOneTeam.

teamSizeCorrect :- minSize(LOW), maxSize(HIGH), team(T), findall( wt(W,T), worker(W), Lits ), atMost(HIGH, Lits), atLeast(LOW, Lits), fail.
teamSizeCorrect.

incompatible :- team(T), findall(wt(W, T), worker(W), Lits), member(wt(W1, T), Lits), member(wt(W2, T), Lits), incompatibleWorkers(W1, W2), atMost(1, [wt(W1, T), wt(W2, T)]), fail.
incompatible. 
