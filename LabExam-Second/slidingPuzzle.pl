...
step(OldBoard, NewBoard) :-
    ...
    append(ColsBefore, ColsAfter, RestRow),
    append(ColsBefore, [x|ColsAfter], NewRow),
    NewRow \= OldRow,
    %% and Rest is the remainder (as in by select/3) of List.

    %append(ColsBefore, [x,A|ColsAfter], NewRow),
...
oneStep(1, OldBoard, NewBoard) :-
    step(OldBoard, NewBoard).
oneStep(1, OldBoard, NewBoard) :-
    transpose(OldBoard, TransBoard),
    step(TransBoard, NewTransBoard),
    transpose(NewTransBoard, NewBoard).
