...
satVariable( work(G, H) ) :- gangster(G), hour(H). % means : "gangster G works on hour H"
...
writeClauses(MaxConsecutiveHours):-
    noTwoTasksSameHour,
    noDifferentTasksConsecHours,
    allGangstersAvailable,
    linkWork,
...
noTwoTasksSameHour :- gangster(G), hour(H), findall(does(G, T, H), task(T), Lits), atMost(1, Lits), fail.
noTwoTasksSameHour.

noDifferentTasksConsecHours :- gangster(G), between(1, 71, H), P is H + 1, task(T), task(S), not(S = T), writeOneClause([-does(G, T, H), -does(G, S, P)]), fail. 
noDifferentTasksConsecHours.

allGangstersAvailable :- gangster(G), hour(H), blocked(G, H), writeOneClause([-work(G, H)]), fail.
allGangstersAvailable.

%linkWork :- gangster(G), hour(H), findall(does(G, T, H), task(T), Lits), writeOneClause([-work(G, H) | Lits]), fail.
%linkWork.

linkWork :- gangster(G), hour(H), task(T), writeOneClause([-does(G, T, H), work(G, H)]), fail.
linkWork.

neededGangsters :- hour(H), task(T), needed(T, H, N), findall(does(G, T, H), gangster(G), Lits), exactly(N, Lits), fail.
neededGangsters.


maxConsecHours(MaxConsecutiveHours) :- gangster(G), Low is 72 - MaxConsecutiveHours, between(1, Low, H), High is H+MaxConsecutiveHours, 
                                       findall(-work(G, P), between(H, High, P), Lits), writeOneClause(Lits), fail.
maxConsecHours(_).
...
costOfThisSolution(M,Cost) :- between(0, 71, Num), Cost is 72 - Num, gangster(G), findall(H, member(does(G, _, H), M), Hours), append([_, Subs, _], Hours), length(Subs, Cost), consec(Subs), !.

consec([]).
consec([_]).
consec([A,B|C]) :- B is A + 1, consec([B|C]).
