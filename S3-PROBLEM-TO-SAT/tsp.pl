...
satVariable( costly(P)) :- position(P). % costly(P) meaning "Path going from position P-1 to position P has cost 1€"
...
eachPositionOnce :- position(P), findall(visited(C, P), city(C), Lits), exactly(1, Lits), fail.
eachPositionOnce.

eachCityOnce :- city(C), C > 1, findall(visited(C, P), position(P), Lits), exactly(1, Lits), fail.
eachCityOnce.

iniAndfinPos :- numCities(N),
      writeOneClause([visited(1, 0)]),
      writeOneClause([visited(1, N)]),
      fail.
iniAndfinPos.

linkCost :- position(P), P > 0, city(C1), city(C2), even(C1), not(even(C2)), Q is P - 1,
      writeOneClause([-visited(C1, Q), -visited(C2, P), costly(P)]),
      writeOneClause([-visited(C2, Q), -visited(C1, P), costly(P)]),
      fail.
linkCost.

maxBudget :- maxCost(K), findall(costly(P), (position(P), P > 0), Lits), atMost(K, Lits), fail.
maxBudget.

adjacencies :- city(C), position(P), numCities(N), P < N, B is P + 1, 
      findall(visited(A, B), (city(A), adjacency(C, J), member(A, J)), Lits),
      writeOneClause([-visited(C, P) | Lits]),
      fail.
adjacencies.
...
displaySol(M) :- position(P), member(visited(City, P), M), write("Position "), write(P), write(" - City "), write(City), nl, fail.
%displaySol(M):- position(P), member(visited(City,P),M), write("City "), write(City), write(": Position "), write(P), nl, fail.
displaySol(_):- nl.
