...
main(G):- 
    ...
    length(ColVars, N), % Square Sides[i] has topleft coordinate (RowVars[i], ColVars[i])
    RowVars ins 1..Big,
    ColVars ins 1..Big,
    insideBigSquare(Big,Sides,RowVars),
    insideBigSquare(Big,Sides,ColVars),
    nonoverlapping(Sides,RowVars,ColVars),
    intercal(RowVars, ColVars, AllVars),
    label(AllVars),
    displaySol(Big,Sides,RowVars,ColVars), halt.

insideBigSquare(_, [], []).
insideBigSquare(N, [X|XS], [P|PS]) :- Limit is N + 1 - X, P #=< Limit , insideBigSquare(N, XS, PS).

intercal([], [], []).
intercal([A|AS], [B|BS], [A,B|X]) :- intercal(AS, BS, X).

nonoverlapping([], [], []).
nonoverlapping([S|SS], [R|RS], [C|CS]) :- noOverlap(S, R, C, SS, RS, CS), nonoverlapping(SS, RS, CS).

noOverlap(_, _, _,[], [], []).
noOverlap(S, R, C, [T|TS], [A|AS], [B|BS]) :- A #>= S + R #\/ A + T #=< R #\/ B #>= S + C #\/ B + T #=< C, noOverlap(S, R, C,TS, AS, BS).
