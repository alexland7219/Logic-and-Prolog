...
distances([ [  0,  20,  75,  45, 200, 210, 220],
	    [ 20,   0,   5,  15, 210, 215, 215],
	    [ 75,   5,   0,  20, 150, 180, 170],
	    [ 45,  15,  20,   0, 200, 205, 200],
	    [200, 210, 150, 200,   0,  10,  20],
	    [210, 215, 180, 205,  10,   0,  10],
	    [220, 215, 170, 200,  20,  10,   0] ]).
...
satVariable( guarded(C, N) ) :- city(C), night(N).  % means "city C has a pharmacy on duty on night N"

%%%%%%%  2. Clause generation for the SAT solver: =============================================

writeClauses:-
    minimunOnDutyPharmaciesEachNight, % done
    blockedNights,                    % done
    noThreeConsecutiveNights,         % done
    maxOnDutyNights,                  % done
    noTwoPharmaciesInSameCity,        % done
    nearbyOnDutyPharmacies,           % done
    linkGuarded,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl, nl, halt.

% a) there must be at least MinPh pharmacies on duty every night
minimunOnDutyPharmaciesEachNight:- minPharmaciesOnDuty(K), night(N), findall(phn(Ph, N), pharmacy(Ph), Lits), atLeast(K, Lits), fail.
minimunOnDutyPharmaciesEachNight.

% b) each pharmacy has a list of nights when it cannot be on duty
blockedNights:- pharmacy(Ph), findall(phn(Ph, N), pharmacyBlockedNight(Ph, N), Lits), exactly(0, Lits), fail.
blockedNights.

% c) no pharmacy can be on duty three consecutive nights
noThreeConsecutiveNights:- pharmacy(Ph), threeConsecutiveNights(N1,N2,N3), writeOneClause([-phn(Ph, N1), -phn(Ph, N2), -phn(Ph, N3)]), fail.
noThreeConsecutiveNights.

% d) no pharmacy can do more than MaxN nights on duty in the total of N nights
maxOnDutyNights:- maxNightsOnDuty(K), pharmacy(Ph), findall(phn(Ph, N), night(N), Lits), atMost(K, Lits), fail.
maxOnDutyNights.
    
% e) pharmacies in the same city cannot be on duty in the same night
noTwoPharmaciesInSameCity:- city(C), night(N), findall(phn(Ph, N), pharmacyInCity(Ph, C), Lits), atMost(1, Lits), fail.
noTwoPharmaciesInSameCity.

% f) every night there must be some on-duty pharmacy less than K Km from any city
nearbyOnDutyPharmacies:- night(N), city(C), nearbyCities(C, LC), findall(guarded(D, N), member(D, LC), Lits), atLeast(1, [guarded(C, N) | Lits]), fail.
nearbyOnDutyPharmacies.

% extra sat variable in order to link the two sat variables phn(Ph, N) and guarded(C, N).
linkGuarded :- city(C), night(N), findall(phn(Ph, N), pharmacyInCity(Ph, C), Lits), writeOneClause([-guarded(C, N) | Lits]), fail.
linkGuarded.
