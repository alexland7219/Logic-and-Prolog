...
satVariable( hasTower(V))   :- village(V).      % meand "village V has a tower"
...
writeClauses(MaxNumTowers):-
    noTowerOutsideVillage,
    oneTowerMaxPerVillage,
    linkHasTower2,
    limitTowers(MaxNumTowers),
    everyVillageWatched,
    significantTowers,
...
noTowerOutsideVillage :- findall(towerPos(I, J), (position(I, J), not(posVillage(_, I, J))), Lits), exactly(0, Lits), fail.
noTowerOutsideVillage.

oneTowerMaxPerVillage :- village(V), findall(towerPos(I, J), posVillage(V, I, J), Lits), atMost(1, Lits), fail.
oneTowerMaxPerVillage.

linkHasTower2 :- village(V), posVillage(V, I, J), writeOneClause([-towerPos(I, J), hasTower(V)]), fail.
linkHasTower2.

significantTowers :- significantVillage(V), writeOneClause([hasTower(V)]), fail.
significantTowers.

limitTowers(MaxNumTowers) :- findall(hasTower(V), village(V), Lits), atMost(MaxNumTowers, Lits), fail.
limitTowers(_).

everyVillageWatched :- village(V), findall(towerPos(I, J), posWatchesVillage(V, I, J), Lits), atLeast(1, Lits), fail.
everyVillageWatched.
...
costOfThisSolution(M,Cost):- findall(hasTower(V), member(hasTower(V), M), Lits), sort(Lits, L1), length(L1, Cost), !.
