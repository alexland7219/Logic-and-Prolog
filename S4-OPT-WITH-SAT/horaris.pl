...
writeClauses(MaxNumProf):-
    ...
    eachCourseNoBannedRoom,
    eachCourseNoBannedProf,
    relateVarsCDHwithCD,
...
eachCourseNoBannedProf :- courseProfessors(C, LP), findall( cp(C, P), (professor(P), not(member(P, LP))), Lits), exactly(0, Lits), fail.
eachCourseNoBannedProf.

eachCourseExactlyOneRoom:- courseRooms(C, LR), findall( cr(C, R), member(R, LR), Lits), exactly(1, Lits), fail.
eachCourseExactlyOneRoom.

eachCourseNoBannedRoom :- courseRooms(C, LR), findall( cr(C, R), (room(R), not(member(R, LR))), Lits), exactly(0, Lits), fail.
eachCourseNoBannedRoom.
...
eachCourseRightNumberOfDays:- course(C), courseHours(C, N), findall(cd(C, D), day(D), Lits), exactly(N, Lits), fail.
eachCourseRightNumberOfDays.

noOverlapCoursesSameYear:- course(A), course(B), not(A = B), courseYear(A, YA), courseYear(B, YB), YA = YB, day(D), hour(H), writeClause([-cdh(A, D, H), -cdh(B, D, H)]), fail. 
noOverlapCoursesSameYear.

noOverlapProfs:- course(C1), course(C2), not(C1 = C2), professor(P), day(D), hour(H), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail. 
noOverlapProfs.

noOverlapRooms:- course(A), course(B), not(A = B), room(R), day(D), hour(H), writeClause([-cr(A, R), -cr(B, R), -cdh(A, D, H), -cdh(B, D, H)]), fail. 
noOverlapRooms.
