...
%%   "the total sum of the path lengths from the root to all its nodes is K"
ipl(T, D, K) :- T = t(_, []), K is D, !.
ipl(T, D, K) :- D1 is D + 1, T = t(_, L), iplsum(D1, L, M), K is M + D.

% It adds together all ipls of sibling nodes.
iplsum(_, [], 0) :- !.
iplsum(D, [T|TS], M) :- ipl(T, D, A), iplsum(D, TS, N), M is A + N.
