...
% aixo es la versió decisional del vertex cover
...
oneVertexPerEdge :- road(C1, C2), writeOneClause([install(C1), install(C2)]), fail.
oneVertexPerEdge.

maxNumStations :- maxStations(K), findall(install(C), city(C), Lits), atMost(K, Lits), fail.
maxNumStations.

%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, write(M), nl, nl, fail.
displaySol(_) :- write("INSTALL GAS STATIONS ON THE FOLLOWING CITIES: "), nl, fail.
displaySol(M):- 
    nl, city(C), member(install(C), M), nl, write("City "), write(C), fail.
