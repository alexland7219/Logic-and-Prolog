...
oneModPerEvent :- event(E), eventModerators(E, MM), findall(em(E, M), member(M, MM), Lits), exactly(1, Lits), fail.
oneModPerEvent.

noAuthorizedMod :- event(E), eventModerators(E, MM), findall(em(E, M), (moderator(M), not(member(M, MM))), Lits), exactly(0, Lits), fail.
noAuthorizedMod.

oneDayPerEvent :- event(E), eventDays(E, DD), findall(ed(E, D), member(D, DD), Lits), exactly(1, Lits), fail.
oneDayPerEvent.

noAuthorizedDay :- event(E), eventDays(E, DD), findall(ed(E, D), (day(D), not(member(D, DD))), Lits), exactly(0,Lits), fail.
noAuthorizedDay.

maxEventsDay :- day(D), maxEventsPerDay(MAX), findall(ed(E, D), event(E), Lits), atMost(MAX, Lits), fail.
maxEventsDay.

maxWorkDays :- moderator(M), maxDaysPerModerator(MAX), findall(md(M, D), day(D), Lits), atMost(MAX, Lits), fail.
maxWorkDays.

linkVars :- event(E), moderator(M), day(D), writeOneClause([-ed(E, D), -em(E, M), md(M,D)]), fail.
linkVars.
