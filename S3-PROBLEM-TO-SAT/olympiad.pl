...
weFrens :- student(S), student(T), not(friends(S, T)),
      writeOneClause([ -chosen(S), -chosen(T)]),
      fail.
weFrens.

exactlyK :- findall(chosen(S), student(S), Lits), numMembersTeam(K), exactly(K, Lits), fail.
exactlyK.

noRepeatedTopic :- student(S), expertise(S,E), student(T), not(S=T), expertise(T,E),
      writeOneClause([-chosen(S), -chosen(T)]),
      fail.
noRepeatedTopic.
